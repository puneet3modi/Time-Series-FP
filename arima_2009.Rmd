...
df_pre_2009 <- df %>% filter(date < as.Date("2009-03-01"))
df_post_2009 <- df %>% filter(date >= as.Date("2009-03-01"))
```
...
ts_pre_2009 <- create_time_series_objects(df_pre_2009, c(1973, 1, 1))
ts_post_2009 <- create_time_series_objects(df_post_2009, c(2009, 3, 1)) 

pre_2009_train <- ts_pre_2009$ts_total_energy_co2_emissions
post_2009_train <- ts_post_2009$ts_total_energy_co2_emissions
```
...
adf.test(pre_2009_train, alternative="stationary")
# Since the data is non-stationary, apply differencing to detrend the data (why in other notebook) 
seasonal_diff <- diff(pre_2009_train, lag=12, differences=1)
```
...
```{r}
non_seasonal_orders <- list(c(1, 3), c(3, 0), c(2, 2))
...
pre_2009_df_filtered <- df_pre_2009 %>% 
...
```{r}
auto.model <- auto.arima(pre_2009_train, seasonal = TRUE)
```

```{r}
fitted_arima_models <- fit_arima_models(pre_2009_train, non_seasonal_orders, seasonal_order_range)
```


```{r}
best_model_pre2009 <- fitted_arima_models[["best_model"]]
checkresiduals(best_model_pre2009)
ljung_box_test <- Box.test(best_model_pre2009$residuals, lag = 20, type = "Ljung-Box")
...
```{r}
fitted_models <- fit_arimax_models(pre_2009_train, xreg, non_seasonal_orders, seasonal_order_range)
```
...
```{r}
# filtering columns
post_2009_df_filtered <- df_post_2009 %>% 
  select('gdp', 'total_renewable_energy_consumption', 'total_renewable_energy_production', 'total_fossil_fuels_production', 'total_fossil_fuels_consumption')

# make numeric, deal with nulls
xreg_post <- post_2009_df_filtered
if (!is.matrix(xreg_post)) {
  xreg_post <- as.matrix(xreg_post)
}
```

### models for post 2009

```{r}
fitted_models_post <- fit_arimax_models(post_2009_train, xreg_post, non_seasonal_orders, seasonal_order_range)
```


Best model: ARIMA2,1,2 x 1,1,1 : AICc = 680.184 


### pre 2005: checkresiduals for best model, ljung box for residuals of best model - make sure white noise

```{r}
best_model_pre2009 <- fitted_models[["best_model"]]
checkresiduals(best_model_pre2009)
ljung_box_test <- Box.test(best_model_pre2009$residuals, lag = 20, type = "Ljung-Box")
print(ljung_box_test)
```
...
```{r}
best_model_post2009 <- fitted_models_post[["best_model"]]
checkresiduals(best_model_post2009)
ljung_box_test <- Box.test(best_model_post2009$residuals, lag = 20, type = "Ljung-Box")
print(ljung_box_test)
```


```{r}
# forecast best model after 2009 Feb policy implementation
forecast_best_model_2009 <- forecast(fitted_models$best_model, xreg = xreg_post)
```

```{r}
# plot forecasted value with the actual values
# Plot the forecast
plot(forecast_best_model_2009, main = "Forecast vs Actual", xlab = "Time", ylab = "Values", ylim = c(300, 550))

# Add the actual values
lines(post_2009_train, col = "red")
