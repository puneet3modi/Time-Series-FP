...
create_time_series_objects <- function(df, start_year) {
  ts_list <- list()
  for (col_name in colnames(df)) {
    if (col_name != "date") {  
      col_data <- df[[col_name]]
      ts_obj <- ts(col_data, start = c(start_year, 1), frequency = 12)
...
Lowest p + q combos:
ARIMA(1, 0, 3), ARIMA(3, 0, 0), ARIMA(2, 0, 2) 
  ...
  for(order in non_seasonal_orders) {
    for(P in (seasonal_order_range$P[1]):(seasonal_order_range$P[3])) {
      for(Q in (seasonal_order_range$Q[1]):(seasonal_order_range$Q[3])) {
        current_order <- c(order[1], 0, order[2])
...
```{r}
auto.arima(pre_2009_train, xreg = xreg, seasonal = TRUE)
```
...
# Plot the forecast
plot(forecast_best_model_2009, main = "Forecast vs Actual", xlab = "Time", ylab = "Values")
